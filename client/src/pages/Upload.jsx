...
function Upload() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      alert(`Upload failed: ${err.message || 'Network error. Please check if the backend server is running.'}`);
    } finally {
        ...
        >
          ...
          <div className="mt-8 text-center">
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              disabled={isUploading || !uploadedFile || (!selectedOptions.summary && !selectedOptions.flashcards && !selectedOptions.quiz)}
              onClick={handleGenerate}
              className={`px-8 py-3 rounded-full font-semibold text-white transition-all 
                ${isUploading || !uploadedFileName || (!selectedOptions.summary && !selectedOptions.flashcards && !selectedOptions.quiz)
